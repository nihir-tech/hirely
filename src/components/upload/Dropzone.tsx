    ...
    <div className="space-y-3">
      <div
        ...
        className={`
          ...
          ${
            disabled
              ? 'opacity-40 cursor-not-allowed border-line bg-surface-subtle'
              : dragOver
                ...
                : error
                  ...
                  : 'border-line bg-surface hover:border-brand-500/30 hover:bg-brand-500/[0.02]'
          }
        `}
      >
        <input
        ...
        <div className={`mb-4 p-4 rounded-2xl transition-all duration-300 ${dragOver ? 'bg-brand-500/10 text-brand-700 dark:text-brand-400' : 'bg-chip text-fg-muted'}`}>
        ...
        <p className="text-base font-medium text-fg-strong mb-1">
...
export function FileCard({ file, preview, onRemove, onReplace }: FileCardProps) {
  ...
  return (
    <div className="flex items-center gap-4 p-4 glass-card">
      {preview ? (
        <div className="w-16 h-20 rounded-xl overflow-hidden bg-chip border border-line shrink-0">
        ...
        <div className="w-16 h-20 rounded-xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center shrink-0">
          <span className="text-xs font-bold text-brand-600 dark:text-brand-400">{typeLabel}</span>
        </div>
      )}
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-fg-strong truncate">{file.name}</p>
        ...
        <div className="flex gap-2 mt-2">
          <button
            ...
            className="text-xs text-brand-600 hover:text-brand-500 dark:text-brand-400 dark:hover:text-brand-300 font-medium"
          >
            Replace
          </button>
